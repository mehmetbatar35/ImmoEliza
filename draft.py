...
class Property:
    def __init__(self):
        ...
        for j in range(1, 334):
            for i in range(2):
                if i == 0:
                    self.url = "https://www.immoweb.be/en/search/house/for-sale?countries=BE&priceType=SALE_PRICE&page={j}&orderBy=relevance"
                else:   
                    self.url = "https://www.immoweb.be/en/search/apartment/for-sale?countries=BE&priceType=SALE_PRICE&page={j}&orderBy=relevance"
            ...
            with open('output.json', 'w', encoding='utf-8') as f:
                ...
                for link in self.property_links:
                    ...
                    if property_json["property"]['kitchen'] and property_json["property"]['kitchen']['type'] != None:
                        prop_dict ["Kitchen Fully Equiped"] = "Yes" if "HYPER" in property_json["property"]['kitchen']['type'] else "No"
                    else:   
                        prop_dict ["Kitchen Fully Equiped"] = "No"
                    prop_dict ["Furnished"] = 'Yes' if property_json['transaction']['sale']['isFurnished'] else 'No'
                    prop_dict ["Open fire"] = "Yes" if property_json['property']['fireplaceExists'] else "No"
                    ...
                    prop_dict ["Swimming Pool"] = 'Yes' if property_json['property']['hasSwimmingPool'] else 'No'
